...
- **已用上**：写时重排的分析 + 属性 + 在 **L2** 的 local 缓冲区维度和访问下标上的应用。
- **待接入**：  
  - **L3_in_serialize**：按重排后的维度顺序生成 DRAM 读（嵌套循环 + 线性下标公式）。  
  - **drain_IO_L3_out_serialize**：写时重排——按重排顺序写回 DRAM（例如 2D 时先填 buffer 再按重排顺序写出）。  
